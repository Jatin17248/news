...
const News = (props) => {
  ...
  const basic = async (bcountry, bpageItem, bpage, bcategory) => {
    ...
    let url = `https://newsapi.org/v2/top-headlines?country=${bcountry}&category=${bcategory}&apiKey=${props.apiKey}&language=en&pageSize=${bpageItem}&page=${bpage}`;
  ...
  const nextPage = async () => {
    let url = `https://newsapi.org/v2/top-headlines?country=${
      ...
      props.category
    }&apiKey=${props.apiKey}&language=en&pageSize=${
      props.pageItem
    }&page=${page + 1}`;
    setPage(page + 1);
    let data = await fetch(url);
          ...
          >
            ...
            <div className="row">
              {articles.map((element) => {
                return element.title !== null && element.urlToImage !== null ? (
